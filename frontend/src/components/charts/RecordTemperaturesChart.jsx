    ...
    <div>
      ...
      <div style={{ marginTop: '16px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '12px' }}>
        ...
        <div style={{ padding: '12px', background: '#fef2f2', borderRadius: '8px', border: '1px solid #fecaca' }}>
          ...
          <p style={{ margin: '0', fontSize: '11px', color: '#7f1d1d' }}>
            {chartData.find(d => d.recordHigh === Math.max(...chartData.map(d => d.recordHigh)))?.displayDate}
          </p>
        </div>
        ...
        <div style={{ padding: '12px', background: '#eff6ff', borderRadius: '8px', border: '1px solid: #bfdbfe' }}>
          ...
          <p style={{ margin: '0', fontSize: '11px', color: '#1e3a8a' }}>
            {chartData.find(d => d.recordLow === Math.min(...chartData.map(d => d.recordLow)))?.displayDate}
          </p>
        </div>
      </div>
      ...
      <div style={{ marginTop: '12px', padding: '12px', background: '#fef3c7', borderRadius: '8px', border: '1px solid #fbbf24' }}>
        <p style={{ margin: 0, fontSize: '11px', color: '#78350f' }}>
          <strong>Note:</strong> Records shown are based on available historical data.
          Actual all-time records may differ if more historical data is available.
        </p>
      </div>
    </div>
  );
