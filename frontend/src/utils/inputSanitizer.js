...
const VALID_KEYWORDS = [
  // Weather terms
  'weather', 'climate', 'temperature', 'temp', 'hot', 'cold', 'warm', 'cool',
  'humid', 'dry', 'wet', 'rain', 'rainy', 'snow', 'sunny', 'cloudy', 'wind',
  'storm', 'fog', 'freeze', 'frost', 'heat', 'chill',

  // Temperature descriptors
  'warmer', 'cooler', 'hotter', 'colder', 'milder', 'moderate', 'extreme',
  ...
  // Precipitation
  'precipitation', 'rainfall', 'drizzle', 'shower', 'downpour', 'monsoon',

  // Location terms
  'city', 'cities', 'location', 'place', 'where', 'area', 'region', 'country',
  'state', 'town', 'live', 'move', 'relocate', 'visit', 'travel',
  ...
  'community', 'beach', 'mountain', 'ocean', 'lake', 'desert', 'tropical',
  'subtropical', 'temperate', 'continental', 'coastal', 'inland',

  // Comparative terms
  'less', 'more', 'better', 'worse', 'like', 'similar', 'different', 'than',
  'compared', 'versus', 'vs',

  // General climate terms
  'sun', 'sunshine', 'shade', 'breeze', 'gust', 'calm', 'clear', 'overcast',
  'muggy', 'sticky', 'crisp', 'fresh', 'stale', 'comfortable'
];
...
export function validateClimatInput(input) {
  ...
  if (trimmed.length < 10) {
    return {
      isValid: false,
      reason: 'Query too short. Please describe your climate preferences in more detail (minimum 10 characters).'
    };
  }
  ...
  const hasLocationPattern = /\b(i want|i need|looking for|find|search|somewhere|anywhere|where can|where should|recommend|suggest)\b/i.test(normalized);
  ...
  if (!hasValidKeyword && !hasLocationPattern) {
    return {
      isValid: false,
      reason: 'Please include climate, weather, or location-related terms in your query. Examples: temperature, humidity, rain, sunny, city, warmer, cooler, etc.'
    };
  }
